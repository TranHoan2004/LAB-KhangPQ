...
public class Validation {
    ...
    public static double getDoubleWithStepOfValue(String mess, String errorNumberFormat, String errorOutOfRange, double min, double max, double step) {
        Scanner scan = new Scanner(System.in);
        while (true) {
            try {
                System.out.print(mess);
                double ret = Double.parseDouble(scan.nextLine());
                if (ret < min || ret > max) {
                    System.err.println(errorOutOfRange);
                } else if (ret % step != 0) {
                    System.err.println("Input must be %" + step + "=0");
                } else {
                    return ret;
                }
            } catch (Exception e) {
                System.err.println("Must be input a number!");
            }

        }
    }

    public static String getDate(String msg, String err, String regex) {
        Scanner scan = new Scanner(System.in);
        String output = null;
        while (true) {
